#Lets look into the tsv big file
library(dplyr)
library(janitor)
library(readr)
library(ggplot2)
...
library(data.table)
library(stringr)
blast <- read.delim("all_taxonomic_hits.tsv", 
                    header = TRUE, 
                    sep = "\t", 
...
nofilter=read.csv("nofilter.csv")
nofilter=nofilter[,-(1:2)]
nofilter <- janitor::clean_names(nofilter)

#good

aster_raw <- nofilter %>%
  filter(otu_family == "Asteraceae" &
           (otu_genus == "Unclassified" | otu_species == "Unclassified"))
aster_map <- aster_raw %>%
  distinct(prey) %>%
  arrange(prey) %>%
  mutate(aster_label = paste0("Asteraceae_", row_number()))

aster_map
nofilter2 <- nofilter %>%
  left_join(aster_map, by = "prey") %>%
  mutate(
    otu_species = ifelse(
      otu_family == "Asteraceae" &
        (otu_genus == "Unclassified" | otu_species == "Unclassified"),
      aster_label,
      otu_species
    )
...
unique(nofilter$prey) #447


otu_table_clean <- read_csv("otu_table_clean.csv")
length(otu_table_clean$OTU)#447

aster <- nofilter %>%
  filter(otu_family == "Asteraceae")
aster_sum <- aster %>%
  group_by(prey) %>%
  summarise(total_reads = sum(ra), .groups = "drop") %>%
  arrange(desc(total_reads))

ggplot(aster_sum, aes(y = reorder(prey, total_reads), x = total_reads)) +
  geom_col(fill = "goldenrod") +
  coord_flip() +
  labs(
    title = "OTUs abundance (first filter)",
    y = "Asteraceae OTU",
    x = "Total Reads"
  ) +
...
aster_pa <- aster %>%
  mutate(pa = ifelse(ra > 0, 1, 0))
aster_pa_sum <- aster_pa %>%
  group_by(prey) %>%
  summarise(n_predators = sum(pa), .groups = "drop") %>%
  arrange(desc(n_predators))


aster_nf <- nofilter %>%
  filter(otu_family == "Asteraceae")
ggplot(aster_nf, aes(y = prey, x = ra)) +
  geom_boxplot(outlier.shape = 16, outlier.size = 1, fill = "darkolivegreen3") +
  coord_flip() +
  labs(
    title = "Distribution of RA per Asteraceae OTU (nofilter)",
    y = "Asteraceae OTU",
    x = "Read Abundance (RA)"
  ) +
  theme_minimal(base_size = 14) +
...
blast_sun = blast[blast$qseqid=="otu1",]
  
blast_sun
blast_sun <- blast_sun %>%
  mutate(species_clean = sub("^([A-Za-z]+ [a-z-]+).*", "\\1", stitle))

#### Before filtering and taxonomic assigment, taxonomic verification of each occurrences. ----

blast <- blast %>%
  mutate(species_clean = sub("^([A-Za-z]+ [a-z-]+).*", "\\1", stitle))
#Now I need the list of species around the sites # The list is a bit too big (in radius, but I monstly need the elevation aswell for the taxonomic validation).

length(unique(blast$species_clean)) #23906 species to check with my filters.

plantsCorv=read.csv("Corviglia.csv",sep=",")
plantsAros=read.csv("Arosa.csv",sep=",")
plantsDist=read.csv("Distelboden.csv",sep=",")
plantsTief=read.csv("Tiefenbach.csv",sep=",")

plants_sites <- rbind(plantsCorv, plantsAros, plantsDist, plantsTief)

# Keep only unique species
# Replace 'species' with the actual column name containing species names
local_plants <- plants_sites[!duplicated(plants_sites$TaxonID), ]

# Check how many unique species you have
nrow(local_plants) # 2249 different local_species of plants. 


#First filter with >1000 meters European plants (692,406) 
#### the file ocurrences.txt is easily accessible on GBIF using filters:
#{
#  "and" : [
#    "Continent is Europe",
...
occurrence <- fread("ocurrences.txt", sep = "\t", quote = "", fill = TRUE)
colnames(occurrence)
...
as.factor(occurrence$occurrenceStatus)

occurrence_clean=occurrence %>%
  select(class,
         order,
         family,
         genus,
         ...
         countryCode,
         year)
nrow(occurrence) # 692406 species >1000m in EU

#ok i have my list of european >1000m species

#lets prefilter all_taxaonomic_hits before the pipline!

#if not in the occurence list, the row in blast is deleted.

head(blast)

head(occurrence_clean[species=="helianthus annuus",]) # ok present naturally in EU apparently at these elevations

head(occurrence_clean[species=="cannabis sativa",]) # not present, should be manually added

occurrence_clean <- occurrence_clean %>%
  add_row(
    class  = "Magnoliopsida",
    order  = "Rosales",
    family = "Cannabaceae",
...
blast <- blast %>%
  mutate(species_clean = str_to_lower(str_trim(species_clean)))

occurrence_clean <- occurrence_clean %>%
  mutate(species = str_to_lower(str_trim(species)))

#Deleting blast rows that are not plausible (species not present in EU above 1200m of elevation)
blast_filtered <- blast %>%
  filter(species_clean %in% occurrence_clean$species)

length(unique(blast_filtered$species_clean))
#456 species now. 


#AND now we proceed with the pipeline! (OTU_assignment)

#this pipeline is following metabinkit 
# =========================================================
# Copyright 2020
#
#
